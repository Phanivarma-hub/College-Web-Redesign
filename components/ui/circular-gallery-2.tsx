...
class Media {
  ...
  createShader() {
    ...
    const img = new Image();
    const isRemote = this.image.startsWith('http');
    if (isRemote) img.crossOrigin = "anonymous";
    img.src = this.image;
    img.onload = () => {
      // Robust texture upload with optional resizing for high-res images
      let processedImage: HTMLImageElement | HTMLCanvasElement = img;
      const maxDim = 2048;
